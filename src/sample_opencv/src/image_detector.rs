...
pub trait ImageFaceDetecterTrait {
    fn run(&mut self, paths: VecDeque<PathBuf>);
    fn stop(&mut self);
    fn join(&mut self);
}

impl ImageFaceDetecterTrait for ImageFaceDetecter {
            ...
            match rx_to_stop_detection.try_recv() {
                ...
                _ => {
                    match rx_detection.recv() {
                        Ok(received) => {
                            println!("ImageFaceDetecter operating...");
